...
! ymbyun 2018/07/11
! NOTE: This might be over-parallelization because parallelization has little (or a negative) effect on a single loop.

! ymbyun 2022/12/27
! Let's keep it simple!
! For a fair comparison of OpenMP and OpenACC
...
! ymbyun 2019/03/20
! From now on, bra(:,:) does not consider memroy affinity for simplicity and safety.
...
!!$OMP END PARALLEL
       case(COHSEX)
! ymbyun 2019/03/14
! From now on, COHSEX is parallelized, too.
! The speed-up is very small here because parallelization has little (or a negative) effect on a single loop.
