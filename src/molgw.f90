...
!=====
 ...
 do istep=1,nstep
   ...
   !
   ! ymbyun 2018/05/07
   ! NOTE: Eliminating small eigenvalues causes big errors in large basis sets (e.g. TZ, QZ, and 5Z)
   ! without the RI approximation.
   !
 ...
 ! NOTE: calc_type is changed from protected to public, which breaks the rule (see m_inputparam.f90).
 ! ymbyun 2019/10/24
 ! BUGFIX: GW:GW:GW is changed to GW:GnW0:GnWn.
 ! ymbyun 2019/10/31
 ! PT2@scCOHSEX is enabled.
#ifdef ENABLE_YMBYUN
 if( calc_type%scf_name == 'COHSEX' ) then
   if     ( calc_type%postscf_name == 'GW'   .OR. calc_type%postscf_name == 'G0W0' ) then
     calc_type%selfenergy_approx    = GW
     calc_type%selfenergy_technique = one_shot
   else if( calc_type%postscf_name == 'GNW0' ) then
     calc_type%selfenergy_approx    = GnW0
     calc_type%selfenergy_technique = EVSC
   else if( calc_type%postscf_name == 'GNWN' .OR. calc_type%postscf_name == 'EVGW' ) then
     calc_type%selfenergy_approx    = GnWn
     calc_type%selfenergy_technique = EVSC
   else if( calc_type%postscf_name == 'PT2'  .OR. calc_type%postscf_name == 'MP2_SELFENERGY' ) then
