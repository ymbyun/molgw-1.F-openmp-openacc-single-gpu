...
#if defined (_OPENMP) && !defined (_OPENACC)
 !$OMP PARALLEL
 !$OMP DO PRIVATE(pstate, qstate, jkspin, jstate,fj,ej, kstate,fk,ek,fact_occ1,fact_occ2,coul_ipkj,coul_iqjk,coul_ijkq,ep,eq,fact_real,fact_energy) REDUCTION(+:emp2_ring,emp2_sox) COLLAPSE(3)
#endif
#if !defined (_OPENMP) && defined (_OPENACC)
